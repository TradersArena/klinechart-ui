...
console.log("-================== ENTERING KLINECHARTPRO.TSX ========================---");
...
import { SymbolInfo, Period, ChartPro, ChartProOptions, openTrade, OrderInfo } from './types'
...
export default class KLineChartPro implements ChartPro {
  constructor (options: ChartProOptions) {
    ...
    console.log("-= KLINE CHART CONSTRUCTOR CALLED =-")
    ...
    render(
      () => (        
        <ChartProComponent
          ...
          openTrades={ 
            options.openTrades ?? [
              { id: 1, sl:66000, e:68000, tp:170000 }]
            }
          name={options.name ?? 'Hi there!'}
        />
      ),
      this._container
    )
    // this._openTrades = options.openTrades ?? [{ sl: 66000, e: 68000, tp: 70000 }]
  }
  ...
  private _openTrades: openTrade[] = [];  // Array to store trading positions



  // Manage open trades directly
  // setOpenTrades(openTrades: openTrade[]): void {
  //   this._openTrades = openTrades
  // }

  // getOpenTrades(): openTrade[] {
  //   return this._openTrades
  // }
  

  // // Method to add a new trading position
  setOpenTrades (openTrades: openTrade[]): void {
    this._chartApi!.setOpenTrades(openTrades)
  }

  // Method to get all trading positions
  getOpenTrades(): openTrade[] {
    return this._chartApi!.getOpenTrades()
  }


  processTrades() {
    
    let openTrades = this._chartApi!.getOpenTrades() as openTrade | openTrade[];
    openTrades = Array.isArray(openTrades) ? openTrades : [openTrades];
    console.log("PROCESSING TRADES: ", openTrades);
    console.log("Length: ", openTrades.length);
    // if (openTrades.length > 0){
      openTrades.forEach(trade => {
        console.log("Order Found :", trade);
        const orderInfo: OrderInfo = {
          orderId: 1, // Assume a method to generate a unique order ID
          action: 'buy', // Example static value, adjust based on actual logic
          entryPoint: trade.e,
          stopLoss: trade.sl,
          takeProfit: trade.tp,
          lotSize: 100, // Example static value, adjust based on actual logic
  ...
  setName (name: string): void {
    this._chartApi!.setName(name)
  }

  getName (): string {
    return this._chartApi!.getName()
  }  
